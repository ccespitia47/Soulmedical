...
export default function HomePage({ onOpenBuilder, onOpenForm, currentUser }: HomePageProps) {
  ...
  // Asignación de usuarios (form o project)
  ...
  const formCountByProject = useMemo(() => {
    ...
    folders.forEach((f) => {
      map.set(f.projectId, (map.get(f.projectId) ?? 0) + f.forms.length);
    });
    return map;
  }, [folders]);
  ...
  // Navegación con tracking de recientes
  ...
  const buildFormActions = (form: { id: string; name: string }) => {
    if (!selectedFolder) {
      return {
        T, isFav: false, onToggleFav: () => {}, onEdit: () => {},
        onDuplicate: () => {}, onAssign: () => {}, onDelete: () => {},
      };
    }
    ...
    return {
      ...
      onAssign: () =>
        openAssignTarget({
          id: form.id, name: form.name, kind: "form", folderId: selectedFolder.id,
        }),
      onDelete: () => setConfirmDelete({ folderId: selectedFolder.id, formId: form.id, formName: form.name }),
    };
  };
    ...
    <div className="flex h-screen overflow-hidden font-sans">
      ...
      {showNewProject && (
        <EntityFormModal
          ...
          onChangeColor={setProjectColor}
          onChangeIcon={setProjectIcon}
      ...
      {assignTarget && (
        <AssignUsersModal
          ...
          subtitle={`${assignTarget.name} · Selecciona quiénes pueden ver este ${
            assignTarget.kind === "form" ? "formulario" : "proyecto"
          }`}
          users={allUsers}
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          message={`Eliminarás "<strong>${confirmDelete.formName}</strong>".`}
      ...
      {confirmDuplicate && (
        <ConfirmModal
          ...
          message={`Se creará una copia de "<strong>${confirmDuplicate.formName}</strong>".`}
      ...
      {confirmDeleteFolder && (
        <ConfirmModal
          ...
          message={`Eliminarás "<strong>${confirmDeleteFolder.folderName}</strong>" y todos sus formularios.`}
          confirmLabel="Eliminar"
          confirmColor="#ef4444"
      ...
      {confirmDuplicateFolder && (
        <ConfirmModal
          ...
          message={`Se duplicará "<strong>${confirmDuplicateFolder.folderName}</strong>".`}
          confirmLabel="Duplicar"
          confirmColor="#3b82f6"
      ...
      {confirmDeleteProject && (
        <ConfirmModal
          ...
          message={`Eliminarás "<strong>${confirmDeleteProject.projectName}</strong>" y todo su contenido.`}
          confirmLabel="Eliminar"
          confirmColor="#ef4444"
