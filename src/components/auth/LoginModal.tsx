...
export default function LoginModal({ app, onClose, onLogin, onGoForgot, onGoRegister }: LoginModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const authUser: AuthUser = { ...user, avatar: ROLE_AVATARS[user.role] ?? "👤" };
      onLogin(authUser, app.id);
    } catch (e) {
      ...
      <div className="animate-modal-in w-full max-w-[420px] rounded-[20px] bg-white px-9 pb-8 pt-9 shadow-[0_24px_60px_rgba(0,40,80,0.3)]">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-extrabold text-slate-900">{app.name}</div>
              <div className="text-xs text-slate-500">Ingresa tus credenciales</div>
            </div>
          </div>
          <button
            onClick={onClose}
